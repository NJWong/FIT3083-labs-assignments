...
//a note on the error handling (notthe error callbacks) in this storage engine
...
storageEngine = function() {
	...
	return {
	    ...
	    findAll : function(type, successCallback, errorCallback) { 
	    	...
	    	var result = [];
	    	var tx = database.transaction(type);		//default param 2 value is just read which suits us fine here
	    	var objectStore = tx.objectStore(type);
			
	    	objectStore.openCursor().onsuccess = function(event) {
	    		var cursor = event.target.result;		//a cursor represents a result set and has methods to navigate to (e.g. continue) and access (e.g. value) all member of the result set
	    		if (cursor) {	//if cursor is null (false) reached end of the result set
	    			result.push(cursor.value);
	    			cursor.continue();
	    		}
				else
	    			successCallback(result);			//done, successCallback passed in by calling code, no need to wait for transaction to complete, the find was successful and its passive
	    	};				
	    },
